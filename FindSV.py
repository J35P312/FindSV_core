...
#install the pipeline
    ...
    parser.add_argument("--no_root",action="store_true",help="used with the auto option, root(5.34.34) will not be installed, and is assumed to be added to path")
    parser.add_argument("--compile_root",action="store_true",help="used with the auto option, used to compile root from source(used if the --auto option fails at cnvnator install step)")
    ...
    if not os.path.exists(os.path.join(programDirectory,"config.txt")):
        if args.UPPMAX:
            ...
            setup.generate_config(programDirectory)
            setup.auto(programDirectory,args)
        else:
